...
# Below this on the LEVEL-BLIND view, a high score was carried by the loudness envelope
# rather than the recording. Calibrated on the ground-truth benchmark, see _eq_invariant_lvl.
ARR_LVL_VETO = 0.030
...
def _eq_invariant_lvl(M):
    """_eq_invariant, but ALSO removes each frame's own level.

    _eq_invariant kills the static EQ (per-band time-mean) and unit-norms each frame, but
    a frame that is simply LOUD still comes out all-positive and a quiet one all-negative,
    and the L2 norm preserves that direction. Two unrelated songs that merely share a
    loudness shape then correlate. Measured on the ground-truth benchmark: 52 of 1008
    different-song pairs cleared the 0.50 crown floor and the worst offenders were all the
    loudness-step variant, two of them at core 1.000.

    Centring per frame as well fixes that - but swapping it in as THE score also crushed a
    real answer (the Mist "speeds+reverb" upload the owner confirmed went 0.848 -> 0.384,
    under the crown floor). So it is not the score. It is a WITNESS: used only to veto a
    high score that this level-blind view cannot see at all.
    """
    if M.shape[0] < 2:
        return M
    M = M - M.mean(axis=0, keepdims=True)
...
def _arr_score_lvl(Mc, Mk, hop=1024, freq_shifts=(-2, -1, 0, 1, 2)):
    """_arr_score computed on the level-blind view. Veto signal only."""
    A = _eq_invariant_lvl(Mc)
    Tc, Tk = A.shape[0], _eq_invariant_lvl(Mk).shape[0]
    if Tc < 4 or Tk < 4:
        return 0.0
    best = 0.0
    for fs in freq_shifts:
        B = _eq_invariant_lvl(np.roll(Mk, fs, axis=1))
        n = min(A.shape[0], B.shape[0])
        if n < 4:
            continue
        v = float(np.mean(np.sum(A[:n] * B[:n], axis=1)))
        if v > best:
            best = v
    return best
...
def verify(clip_path, cand_path, seconds=20, clip_ctx=None):
    ...
    arr_lvl = _arr_score_lvl(clip_ctx["spec"], Sk)
    out["arr_lvl"] = round(arr_lvl, 4)
    out["arr"] = round(arr, 4)
    out["lag"] = round(lag, 2)
    ...
    # LEVEL-BLIND VETO. A pair that scores high here but reads as NOTHING once each
    # frame's own loudness is removed was matched on the loudness envelope, not on the
    # recording. Real matches survive both views: on the benchmark every true pair that
    # scored >= 0.50 kept arr_lvl well clear of this floor, while the different-song pairs
    # that cleared the crown floor collapsed. Damped rather than zeroed, so a borderline
    # real match is demoted instead of erased.
    if core >= 0.50 and arr_lvl < ARR_LVL_VETO:
        core *= 0.35
        out["level_vetoed"] = True
